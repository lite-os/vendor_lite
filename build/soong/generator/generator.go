...
func init() {
	android.RegisterModuleType("lite_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = liteExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = liteExpandVariables(ctx, dep_file)
	...
	cmd := liteExpandVariables(ctx, String(g.properties.Cmd))

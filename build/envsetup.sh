# LiteOS functions that extend build/envsetup.sh
function __print_lite_functions_help() {
cat <<EOF
Additional LiteOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- literemote:       Add a git remote for LiteOS github repository.
...
function lite_device_combos()
{
    local T list_file variant device

    T="$(gettop)"
    list_file="${T}/vendor/lite/lite.devices"
    variant="userdebug"
    ...
    do
        add_lunch_combo "lite_${device}-${variant}"
    done < "${list_file}"
}

function lite_rename_function()
{
    eval "original_lite_$(declare -f ${1})"
}

function _lite_build_hmm() #hidden
{
    printf "%-8s %s" "${1}:" "${2}"
}

function lite_append_hmm()
{
    HMM_DESCRIPTIVE=("${HMM_DESCRIPTIVE[@]}" "$(_lite_build_hmm "$1" "$2")")
}

function lite_add_hmm_entry()
{
    for c in ${!HMM_DESCRIPTIVE[*]}
    do
        if [[ "${1}" == $(echo "${HMM_DESCRIPTIVE[$c]}" | cut -f1 -d":") ]]
        then
            HMM_DESCRIPTIVE[${c}]="$(_lite_build_hmm "$1" "$2")"
            return
        fi
    done
    lite_append_hmm "$1" "$2"
...
function literemote()
{
    ...
    git remote add lite "https://github.com/lite-os/$project"
    echo "Remote 'lite' created"
}
...
function litepush()
{
    ...
    git $path_opt push "https://github.com/lite-os/$proj" "HEAD:$branch"
}


lite_rename_function hmm
function hmm() #hidden
{
    local i T
    T="$(gettop)"
    ...
    echo "vendor/lite extended functions. The complete list is:"
    for i in $(grep -P '^function .*$' "$T/vendor/lite/build/envsetup.sh" | grep -v "#hidden" | sed 's/function \([a-z_]*\).*/\1/' | sort | uniq); do
        echo "$i"
    done |column
}
...
function breakfast()
{
    target=$1
    local variant=$2
    LiteOS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/lite/build/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the LiteOS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch lite_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var LiteOS_MOD_VERSION)
        ZIPFILE=lite-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.gs.device | grep -q "$LiteOS_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $LiteOS_BUILD, run away!"
    fi
}
...
function installboot()
{
    ...
    if (adb shell getprop ro.lite.device | grep -q "$LiteOS_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        literemote
        git push lite HEAD:refs/heads/'$1'
    fi
    '
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.lite.device | grep -q "$LiteOS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
alias litekap='dopush litemka'

function repopick() {
    T=$(gettop)
    $T/vendor/lite/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $LiteOS_FIXUP_COMMON_OUT ]; then
